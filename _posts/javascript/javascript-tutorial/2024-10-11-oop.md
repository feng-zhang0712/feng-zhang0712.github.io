...
函数实例的 `call(thisArg, arg1, arg2, ...)` 方法可以指定函数内部 `this` 的指向。第一个参数是 `this` 所要指向的那个对象，后面的参数是函数调用时所需的参数。
...
`apply(thisArg, [arg1, arg2, ...])` 方法的作用与 `call()` 方法类似，也是改变 `this` 指向，然后再调用该函数。唯一的区别就是，它接收一个数组作为函数执行时的参数。

`apply()` 方法的一个常见的应用，是利用数组对象的 `slice()` 方法将一个类似数组的对象（比如 `arguments` 对象）转为真正的数组。
...
`bind(thisArg, arg1, arg2, ...)` 方法用于将函数体内的 `this` 绑定到某个对象，然后返回一个**新函数**。`bind()` 方法有一些使用注意点。
...
`Object.setPrototypeOf(obj, prototype)` 方法为参数对象设置原型，返回该参数对象。它接受两个参数，第一个是现有对象，第二个是原型对象。
...
`Object.create(proto, propertiesObject)` 方法接受一个对象作为参数，然后以它为原型，返回一个实例对象。该实例完全继承原型对象的属性。
...
实例对象的 `isPrototypeOf(object)` 方法，用来判断该对象是否为参数对象的原型。
