...
- 扩展运算符（`...`）、`Array.from()`、`keys()`、`values()`、`entries()`、`find()` 和 `findIndex()` 都会将数组的空位，转为 `undefined`。
- `copyWithin()` 会连空位一起拷贝。
- `fill()` 会将空位视为正常的数组位置。
- `for...of` 循环也会遍历空位。
...
## 七、find()，findIndex()，findLast()，findLastIndex()
...
## 十三、toReversed()，toSorted()，toSpliced()，with()
