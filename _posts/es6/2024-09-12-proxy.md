...
`construct()` 方法用于拦截 `new` 命令。`construct()` 方法接受三个参数。
...
**`has()` 方法用来拦截 `HasProperty` 操作**，当判断对象是否具有某个属性时，这个方法会生效。典型的操作是 `in` 运算符。`has()` 方法可以接受两个参数，分别是目标对象、需查询的属性名。
...
`preventExtensions` 方法拦截 `Object.preventExtensions` 方法。该方法必须返回布尔值，否则会被自动转为布尔值。
...
上面代码中，`proxy.preventExtensions` 方法返回 t`rue`，但这时 `Object.isExtensible(proxy)` 会返回 `true`，因此报错。
...
- 虽然 Proxy 可以代理针对目标对象的访问，但它不是目标对象的透明代理，即不做任何拦截的情况下，无法保证与目标对象的行为一致。主要原因是在 Proxy 代理的情况下，目标对象内部的 `this` 关键字会指向 Proxy 代理。
