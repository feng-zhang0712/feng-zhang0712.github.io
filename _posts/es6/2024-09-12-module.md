...
对于模块加载方案，在 ES6 之前主要有 CommonJS 和 AMD 两种。前者用于服务器，后者用于浏览器。CommonJS 和 AMD 模块加载方式属于**运行时加载**或**动态加载**。
...
浏览器加载 ES6 模块，要在 `<script>` 标签加入 `type="module"` 属性。浏览器对于带有 `type="module"` 的 `<script>`，都是异步加载，不会造成堵塞浏览器，即等到整个页面渲染完，再执行模块脚本，等同于打开了 `<script>` 标签的 `defer` 属性。
