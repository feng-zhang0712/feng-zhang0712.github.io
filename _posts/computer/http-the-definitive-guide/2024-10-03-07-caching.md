...
$server freshness limit = $default_cache_min_lifetime;
$heuristic = 1;
if （$heuristic）
{
...
{
$age_limit =
server_freshness_limit（ ）%3B %23%23 From Example 7-2
...
=
$INT_MAX; }
{ $age_limit = server freshness limit（） + $Max_Stale_value; }
if （$Min_Fresh_value_set）
{
$age_limit =
min（$age_limit， server_freshness_limit （ ）
$Min_Fresh_value_set）;
}
if （$Max_Age_value_set）
{
